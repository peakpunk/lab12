...
int findYugiAction(int s){ 
 if(s == 9) return 2; // Yugi will definitely stay (2) when current score (s) is equal to 9
  ...
  if (((rand()%100+1)>=1)&&((rand()%100+1) <= 69)) return 1;
  else return 2;
 }
}
...
int main(){
 ...
 do{
  ...
  cin >> playerAction;
 }while(playerAction != 1 and playerAction != 0); //This line of code is not completed. You need to set the condition to do loop if user's input is not 1 or 2. 
 if(playerAction == 1){
